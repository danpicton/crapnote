...
// Only UUID-named relative paths can identify missing exporter bundles (image
// IDs are generated by images.NewID). Ordinary links such as images/logo.png
// are left untouched by export.Build and must also survive import unchanged.
// External and root-relative URLs are not bundle references either.
var bundledImagePath = regexp.MustCompile("(?:^|[\\s(<\"'`=:])images/[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}\\.[A-Za-z0-9]+")
