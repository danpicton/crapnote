<script lang="ts">
	...
	import { answerCacheGateQueries } from '$lib/sw-cache-gate';
	...
	/**
	 * Vouch for the service worker's cached note images. The SW caches them
	 * and cannot see the auth store, so it asks before serving one and
	 * refuses if nobody answers — without a page here to say the session may
	 * read the cache, a cached image URL would be readable by whoever opens
	 * the browser next (#108).
	 */
	onMount(() => answerCacheGateQueries(() => auth.canReadCache));
