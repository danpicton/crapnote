...
- **Title says where, not just what.** Prefix with the area in brackets — `[backend/notes]`, `[frontend]`, `[editor]`, `[e2e]`, `[deploy]`, `[docs]`. A panel of issues should be navigable by title alone.
...
- **Name the test layer.** This repo works TDD-first; say which layer the work lands in — Go handler/service/repository tests in `backend/internal/`, Vitest unit tests in `frontend/src/`, or Playwright E2E in `e2e/tests/` — and what proves it works.
- **Scope narrowly.** "Fix the error handling" is a bad issue; "return 404 instead of 500 when fetching another user's note in `backend/internal/notes/handler.go`" is a good one. Split rather than broaden.
