...
Offline conflict recovery retains the stricter privacy observed locally or on the server, including across queued flag changes and failed retries. Both conflict copies and private content written back to the original are protected; sync never implicitly clears privacy. Duplicating a private note creates a private copy atomically.
